...
class vaihingenloader(Dataset):
    '''
    ISPRS dataset reader
    '''

    def __init__(self, root, split='train', crop_size=512, scale=False, rotate=True, HorizontalFlip=True,
                 normalize=False, degree=10):
        super().__init__()
        self.root = root
        self.split = split
        ...
        for label_fp in os.listdir(os.path.join(self.root, self.split, 'Urban', 'images_png')):
            # Label path
            label_path = os.path.join(self.root, self.split, 'Urban',  'mask', label_fp)
            # image_fp = label_fp.replace("_noBoundary", "")
            # IRRG img path
            image_path = os.path.join(self.root, self.split, 'Urban', 'images_png', label_fp)

            self.image_list.append(image_path)
            self.label_list.append(label_path)

        for label_fp in os.listdir(os.path.join(self.root, self.split, 'Rural', 'images_png')):
            # Label path
            label_path = os.path.join(self.root, self.split, 'Rural',  'mask', label_fp)
            # image_fp = label_fp.replace("_noBoundary", "")
            # IRRG img path
            image_path = os.path.join(self.root, self.split, 'Rural', 'images_png', label_fp)

            self.image_list.append(image_path)
            self.label_list.append(label_path)

        # for label_fp in os.listdir(os.path.join(self.root, self.split, 'label_nob')):
        #     # Label path
        #     label_path = os.path.join(self.root, self.split, 'label_nob', label_fp)
        #     image_fp = label_fp.replace("_noBoundary", "")
        #     # IRRG img path
    ...
    def encode_segmap(self, mask):
        ...
        # label_mask[label_mask == 5] = 255
        label_mask[label_mask == 7] = 255
...
class vaihingenloader_tg(Dataset):
    '''
    ISPRS dataset reader
    '''
...
if __name__ == '__main__':
    ...
    root = '/media/ssd/bhw/ISPRS2021/data/Vaihingen_data/'

    isprs = vaihingenloader(root=root, split='train')
    print(len(isprs))
    dataloader = DataLoader(isprs, batch_size=8, shuffle=True, num_workers=2, drop_last=True)
